# -*- coding: utf-8 -*-
"""
Created on Sun Nov  1 18:12:23 2025
...
TARGET_NAMES = random.sample(subset_classes, 10)
...
def reverse_gradient_descent_obf(model, loader, lr_neg=LR_NEG, epochs=EPOCHS_RGD):
    ...
    for epoch in range(epochs):
        total_loss = 0
        for imgs, labels in tqdm(loader, desc=f"RGD+Obf Epoch {epoch+1}/{epochs}"):
            obf_imgs = torch.stack([
                transform(obfuscate_image(TF.to_pil_image(i.cpu()))) for i in imgs
            ])
            imgs, labels = obf_imgs.to(DEVICE), labels.to(DEVICE)
        ...
        print(f"RGD+Obf Epoch {epoch+1}/{epochs} | Reverse Loss={-total_loss/len(loader):.4f}")
...
def run_unlearning_experiment(model_name, model_builder, model_path):
    ...
    rgd_time = reverse_gradient_descent_obf(model, target_loader)
    ...
    save_path = os.path.join(MODEL_DIR, f"{model_name}_lfw_unlearning_rgd_obf.pth")
    torch.save(model.state_dict(), save_path)

    csv_path = os.path.join(RESULTS_DIR, "rgd_obf_unlearning_log.csv")
    fieldnames = ["model_name", "target_classes", "acc_before_target", "acc_after_target",
                  "acc_before_retain", "acc_after_retain", "delta_target", "delta_retain", "rgd_time_sec"]
